...
class LoadMyGamesViewModel(private val gameRepository: GameRepository): BaseViewModel(), LifecycleObserver {
    ...
    fun loadMyGames(){
        ...
        gameRepository.loadMyGames()
            .subscribe({ baseModel ->
                viewStateGame.postValue(ViewStateModel(status = ViewStateModel.Status.SUCCESS, model = baseModel.data.list as ArrayList<Media>))
            },{
                viewStateGame.postValue(ViewStateModel(status = ViewStateModel.Status.SUCCESS, errors = notKnownError(it)))
            })
    }
